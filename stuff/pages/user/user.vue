  ...
  <view class="container">
    <view class="user-info">
      <view class="avatar" @click="chooseAvatar">
        <image
          :src="profile.avatar || '/static/avatar.png'"
          mode="aspectFill"
        ></image>
      </view>
      <view class="info">
        ...
        <view class="role-info">
          ...
          <text
            class="family-role"
            v-for="(role, index) in profile.familyRoles"
            :key="index"
          >
    ...
    <view class="menu-list">
      <view
        class="menu-item"
        v-if="canManageItems"
        @click="navigateTo('/pages/category/category')"
      >
        <text>分类管理</text>
      ...
      <view class="menu-item" @click="showJoinFamilyPopup">
        <text>加入家庭组</text>
        <text class="arrow">></text>
      </view>
      ...
      <view class="favorites-popup">
        ...
        <scroll-view class="favorites-list" scroll-y>
          ...
          >
            <view
              class="item-content"
              @click="navigateToDetail(favorite.item.id)"
            >
              <image
                class="item-image"
                :src="favorite.item.imageUrl || '/static/default-item.png'"
                mode="aspectFill"
              ></image>
              <view class="item-info">
                <text class="item-name">{{ favorite.item.name }}</text>
                <text class="item-category">{{
                  favorite.item.category?.name || '未分类'
                }}</text>
                <text class="item-quantity"
                  >数量：{{ favorite.item.quantity }}</text
                >
                <text class="item-location"
                  >位置：{{ favorite.item.location || '暂无' }}</text
                >
              </view>
            </view>
        ...
        <scroll-view class="reminders-list" scroll-y>
          ...
          <view v-if="stockReminders.length > 0" class="reminder-section">
            ...
            >
              ...
              <view class="reminder-content">
                <text class="threshold"
                  >库存阈值：{{ reminder.threshold }}</text
                >
                <text class="current"
                  >当前库存：{{ reminder.currentQuantity }}</text
                >
              </view>
            </view>
          </view>
          ...
          <view v-if="usageReminders.length > 0" class="reminder-section">
            ...
            >
              ...
              <view class="reminder-content">
                <text class="interval"
                  >使用间隔：{{ reminder.interval }}天</text
                >
                <text class="last-usage"
                  >上次使用：{{ formatTime(reminder.lastUsageTime) }}</text
                >
              </view>
            </view>
          </view>
    ...
    <uni-popup ref="profilePopup" type="bottom">
      <view class="profile-popup">
        ...
        <view class="profile-form">
          ...
          <button class="submit-btn" @click="handleUpdateProfile">
            保存修改
          </button>
        </view>
      </view>
    ...
    <uni-popup ref="passwordPopup" type="bottom">
      <view class="password-popup">
        ...
        <view class="password-form">
          ...
          <button class="submit-btn" @click="handleUpdatePassword">
            确认修改
          </button>
        </view>
      </view>
    ...
    <!-- 加入家庭组弹窗 -->
    <uni-popup ref="joinFamilyPopup" type="bottom">
      <view class="join-family-popup">
        <view class="popup-header">
          <text class="title">加入家庭组</text>
          <text class="close" @click="hideJoinFamilyPopup">×</text>
        </view>

        <view class="join-family-form">
          <view class="form-item">
            <text class="label">邀请码</text>
            <input
              type="text"
              v-model="inviteCode"
              placeholder="请输入家庭组邀请码"
              maxlength="6"
            />
          </view>

          <button class="submit-btn" @click="handleJoinFamily">
            加入家庭组
          </button>
        </view>
      </view>
  ...
  methods: {
    ...
    async loadReminders() {
      try {
        ...
        this.hasReminders =
          this.stockReminders.length > 0 || this.usageReminders.length > 0
      } catch (e) {
        uni.showToast({
          title: '获取提醒列表失败',
          icon: 'none',
    ...
    showJoinFamilyPopup() {
      this.$refs.joinFamilyPopup.open()
    },
    hideJoinFamilyPopup() {
      this.$refs.joinFamilyPopup.close()
    },
    async handleJoinFamily() {
      try {
        await userApi.joinFamily(this.userId, this.inviteCode)
        uni.showToast({
          title: '加入家庭组成功',
          icon: 'success',
        })
        this.hideJoinFamilyPopup()
        this.loadUserProfile()
      } catch (e) {
        uni.showToast({
          title: e.message || '加入家庭组失败',
          icon: 'none',
        })
      }
    },
